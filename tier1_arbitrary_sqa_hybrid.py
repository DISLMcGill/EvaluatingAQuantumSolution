"""
Tier-1 ARBITRARY-PARTITION sweep with S1 + S2 *hybrid* encodings.

Hybrid analog of ``tier1_arbitrary_sqa_hw.py``, and the arbitrary-bank
sibling of ``tier1_unit_sqa_hybrid.py``.  Each BQM is submitted to a
D-Wave Leap *hybrid* BQM solver (``LeapHybridSampler``) rather than to a
bare QPU.

The arbitrary-partition bank gives every partition its own integer
``size_p``.  Both encodings handle this: S2's unbalanced penalty takes
``size_p`` in its coefficients, and S1 (slack-variable) reaches feasible
states because its k-safety multiplier is scaled by ``max(size_p)**2``
(see solvers/simulated_solvers/SQA.py -- the hybrid S1 solver inherits
that BQM unchanged).

What this runs
--------------
Tier 1 has 12 (n, p) combinations crossed with 3 tightness levels =
36 configurations, each with 5 instance JSONs (180 total).

  * Default (subset):       one instance per (n, p, tightness) leaf
                            (the ``*_1.json`` file) -- 36 cases.
  * ``--full``:             every instance in every config -- 180 cases.
...
Selection policy
----------------
Defaults to ``best_feasible`` -- the lowest-energy *feasible* sample,
with fallback -- matching the QPU and simulated banks so the hybrid
results are directly comparable.  ``--legacy-lowest-energy`` restores the
old lowest-energy-unconditionally behaviour and writes to the original
hybrid_results/ tree instead of hybrid_results_feasible/.

Why no anneal / num_reads / chain_strength knobs
------------------------------------------------
A Leap hybrid solver hides embedding, reads and anneal schedule and
chooses them itself.  The only client-side knob is ``time_limit``
(seconds); leaving it unset lets the service pick its per-problem
minimum (its floor at tier-1 scale).  Pass ``--time-limit`` to override.

Leap footprint
--------------
Two hybrid submissions per case.  Hybrid solvers bill a minimum charge
time per call (a few seconds) regardless of problem size:
...
-----
    ...
    python tier1_arbitrary_sqa_hybrid.py                         # subset (36 cases), interactive
    python tier1_arbitrary_sqa_hybrid.py --no-pause              # subset, unattended
    python tier1_arbitrary_sqa_hybrid.py --full --no-pause       # full tier-1 (180 cases)
    python tier1_arbitrary_sqa_hybrid.py --time-limit 5 --no-pause  # force 5 s per submission
    python tier1_arbitrary_sqa_hybrid.py --full --no-pause --resume # continue an interrupted sweep
...
# banks.  Unit and arbitrary hybrid runs share these directories but are
# kept distinct by file_prefix (ArbTier1*_HYB vs the unit Tier1*_HYB).
...
# A "tier-1 configuration" is a (n, p, tightness) triple, 1:1 with a leaf
# directory under tier1/ (e.g. n3_p4/t30/).  We take the
# lexicographically-first JSON in each leaf, the ``*_1.json`` instance.
...
def _bench_label(bench_root_arg) -> str:
    """
    Result-file prefix label for the bench being run (arbitrary side).

    Keeps the historical 'ArbTier1' default; for a custom root, labels by
    the bench directory with an 'Arb' prefix so arbitrary stress runs stay
    distinct from the unit stress runs in the shared result tree (e.g.
    test_bank/hybrid_stress/arbitrary_partition -> 'ArbHybridStress').
    """
    ...
    return f"Arb{cleaned}" if cleaned else "ArbBench"
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Tier-1 ARBITRARY-partition sweep on D-Wave Leap hybrid "
                    "solvers: ILP baseline + S1 (SQA_H) + S2 (SQA_SF_H).  "
                    "Hybrid path has no anneal/reads/chain knobs; the only "
                    "tunable is --time-limit (seconds).",
    )
    parser.add_argument(
        "--no-pause",
        action="store_true",
    ...
    parser.add_argument(
        ...
        help="Directory containing n{N}_p{P}/t{TT}/ leaves to run instead "
             "of the default arbitrary-partition tier-1 bank.  Relative "
             "paths resolve against the repo root.  For the stress bench "
             "pass test_bank/hybrid_stress/arbitrary_partition.",
    )
    parser.add_argument(
        "--no-ilp",
        action="store_true",
    ...
    # Distinct prefix from the unit hybrid script so the two sweeps' result
    # files do not collide in the shared hybrid result tree.
    ...
    output_path = run_experiment(
        ...
        resume=args.resume,
        note=(
            f"tier1_arbitrary_sqa_hybrid.py [{mode_label}] -- "
            f"{len(cases)} arbitrary-partition cases on Leap hybrid "
            f"solvers: ILP baseline + {hyb_label}. time_limit={tl_label}.  "
